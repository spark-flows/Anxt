...
class DeviceRepository extends DomainRepository {
  ...
  void deleteBox() async {
    if (Hive.isBoxOpen(StringConstants.appName)) {
      var box = Hive.box(StringConstants.appName);
      await box.clear();
    } else {
      var box = await Hive.openBox(StringConstants.appName);
      await box.clear();
    }
  ...
  /// return bool value
  @override
  bool getBoolValue(String key) => _getBox().get(key, defaultValue: false) as bool;
