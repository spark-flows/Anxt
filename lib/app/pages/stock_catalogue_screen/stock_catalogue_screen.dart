...
class StockCatalogueScreen extends StatelessWidget {
  const StockCatalogueScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return GetBuilder<StockCatalogueController>(
      initState: (state) {
        var controller = Get.find<StockCatalogueController>();
        controller.postAllFolders('');
        controller.selectAllFolder = controller.folderDocList.every(
          (item) => item.isSelect,
        );
                                        ...
                                        fillColor:
                                            ...
                                            >((states) {
                                              if (states.contains(
                                                ...
                                                return ColorsValue.lineColor;
                                              }
                                              return ColorsValue.whiteColor;
                                            }),
                        ...
                        Expanded(
                          child: GridView.builder(
                            ...
                            itemBuilder: (context, index) {
                              final element = controller.folderDocList[index];
                              return Stack(
                                children: [
                                  GestureDetector(
                                    onLongPress: () {
                                      if (controller.isFolderLongPress) {
                                        controller
                                            .folderDocList[index]
                                            .isSelect = true;
                                        controller.isFolderLongPress = false;
                                      } else {
                                        controller
                                            .folderDocList[index]
                                            .isSelect = true;
                                    ...
                                    onTap: () {
                                      if (!controller.isFolderLongPress) {
                                        RouteManagement.goToStockCatalogueDetailScreen(
                                          // controller
                                          //         .folderDocList[index]
                                          //         .id ??
                                          //     "",
